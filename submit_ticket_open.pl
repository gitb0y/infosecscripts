...
if (!$readconfig){
    ...
    $cfg->param("domain_username", 'mydomain\domain_username'); #username to login to CA Service Desk
    $cfg->param("domain_password", 'domain_password');
    $cfg->param("qualys_username", 'qualys_username');
    $cfg->param("qualys_password", 'qualys_password' );
    $cfg->param("tickets_path", '//path\to\submitted tickets'); #folder where the submitted tickets spreadsheets will be stored
    $cfg->param("region_ip_blocks", '//path\to\asset group IPs'); #folder where the exploded IP address range text files will be stored
    $cfg->param("ip_mappings", '//path\to\ip_mappings.xlsx'); #spreadsheet containing External-VIP-ServerIP mappings
    $cfg->param("cve_data", '//path\to\cve_data.xlsx'); #spreadsheet containing IP-to-CVE mappings for marking false positives vulns.
    $cfg->param("xml_folder", '//path\to\xml scan results'); #folder where xml scan results will be cached
    $cfg->param("assignee_group", 'Assignee Group'); #default assignee group where SD tickets will be assigned
    $cfg->param("affected_customer", 'Affected User'); #default affected user that will be indicated in SD tickets.
    $cfg->param("request_area", 'Request Area'); #default request area where SD tickets will be placed.
    $cfg->param("process_mode", []); #whether you want to process scheduled scan results with target asset groups or just adhoc scans with individual IP targets
    $cfg->param("process_nonpci", []); #whether to skip non-pci vulnerabilities when processing scan results
    $cfg->param("selected_vulns", []); #default value in vulnerability selection (#1-7)
    $cfg->param("selected_regions", []); #which asset groups you wish to process.  
    $cfg->param("latest_result_date", []); ### target date of scan that you wish to process the scan result of in YYYY-MM-DD format
    $cfg->param("month_filter", []);  ### include only scan results for the month specified in MM format
    $cfg->param("notify_emails", []); ### whom to send the spreadsheet reports to. (TO:,CC1:,CC2:...)  
    $cfg->param("max_scan_results", []); 
    $cfg->param("log_file", '//path\to\vulns_report.log'); #logfile to write the vulnerability statistics to. (to be fed into Splunk).
    $cfg->param("write_log", 'no'); ### whether to write the logfile or not.
    $cfg->write("qualys.conf");  ### the configuration file to be read by the script. This will be created on initial run with the default values above.
}
...
GetOptions ('target-date:s' => \$target_date, 'process-mode:i' => \$process_mode, 'selected-vuln:s@' => \$origselectedvulns, 'notify-email:s@' => \$notify_emails, 'selected-region:s@' => \$selectedregions, 'process-nonpci:s' => \$process_nonpci, 'assignee-group:s' => \$assignee_group, 'write-log:s' => \$write_log, 'log-file:s' => \$log_file);
...
if (ref($qualysusername)){
   say "SET QUALYS USERNAME IN THE CONFIG FILE BEFORE PROCEEDING. EXITING..\n\n\n";
   $cfg->param("qualys_username", []);
   exit;
}
if (ref($qualyspassword)){
    say "SET QUALYS PASSWORD IN THE CONFIG FILE BEFORE PROCEEDING. EXITING..\n\n\n";
    $cfg->param("qualys_password", []);
    exit;
}
...
if (!defined $notify_emails or $notify_emails eq ''){
   my @notify_emails = $cfg->param("notify_emails");
    if (scalar @notify_emails == 0){
        ...
        chomp $login_name;
        $login_name =~ s/mydomain\\//; #changethis
        $notify_emails = $login_name . '@company.com'; #changethis
   }else{
...
(my $mailfrom) = $username =~ /mydomain\\(.*?)$/; #changethis
$mailfrom .= '@company.com'; #changethis
...
if (ref ($cvedata)){
   $cfg->param("cve_data", '//path\to\cve_data.xlsx'); #changethis
   $cvedata = $cfg->param("cve_data");
}
...
if (ref ($ipmappings)){
   $cfg->param("ip_mappings", '//path\to\ip_mappings.xlsx'); #changethis
   $ipmappings = $cfg->param("ip_mappings");
}
...
my $async = HTTP::Async->new (('proxy_host' => '127.0.0.1', 'proxy_port' => '9090', 'slots' => '20')); ### THIS SCRIPT USES HTTP::Async to retrieve and update SD ticket status in the spreadsheet reports. A proxy with NTLM credentials set for Service Desk server must be running.
...
###################################################################
### THIS USED TO BE A CALL TO IGNORED VULNERABILITY SCORECARD, HOWEVER, IF THIS THREAD IS REMOVED, SCRIPT HALTS (PRESUMABLY BECAUSE OF SOME THREADING CONFLICT)
### SO IT WAS CHANGED TO CALL THAT DOES NOT INCUR ANY COST IN OUR API CALLS LIMIT

my ($loginthread) = threads->create('get_login');
...
unless (! ref ($process_mode)){
       print "\n\n\n1.) Process scheduled scan\n2.) Verify remediation (no tickets)\n\n\nWhat do you want to do?[1] ";
       ReadMode('normal');
...
unless (-e $ticketspath && -d $ticketspath){
  ...
  mkdir $ticketspath or (die "Unable to create submitted tickets folder. Check your permission on $ticketspath\n");
   ...
   $cfg->param("tickets_path", '//path\to\submitted tickets'); #changethis
   $ticketspath = $cfg->param("tickets_path");
}
...
#7.) IF IT IS NOT SELECT MANUALLY, THEN JUST CREATE THE TICKET.
             ...
             if ((defined $vulnticketurlid && $vulnticketurlid ne '') && $process_mode == 1){  
...
#   my $headingstartcell = $qidstartcell + 2;
   ...
   $uniqsheet->write($headingstartcell, 8, ($process_mode == 1?"OLA / Delay":"OLA"), $headingformat);
...
#############################################
              ...
              $uniqsheet->write($affectedstartcell, 5, ($$ticketinfo{age} == 0?'':$$ticketinfo{age}), $format3) unless ${$qid_perregion{$region}}{$qid}[1]{$ip_port}[0] =~ /no ticket yet/;
...
IT_Security_Team_changethis has done a vulnerability assessment on all the hosts in the following regions. The table below provides a summary of the result:
</p>
...
# IN ANOTHER ASSET GROUP. E.G., 'EXTERNAL HOSTS PROD' IS A SUBSET OF 'EXTERNAL HOSTS', THE LIST OF SCANS THAT WILL BE RETRIEVED INCLUDES EXTERNAL HOSTS AS THE TARGET 
# INSTEAD OF JUST EXTERNAL HOSTS PROD.
...
if (ref ($region_ip_blocks)){
   $cfg->param("region_ip_blocks", '//path/to/asset group IPs'); #changethis
   $region_ip_blocks = $cfg->param("region_ip_blocks"); 
}
...
sub get_sid{
  ...
  my $username = shift @_;
  my $password = shift @_; 
  $$ua->credentials('servicedesk_hostname_changethis:80', '', $$username, $$password);
  $$ua->cookie_jar({});
  ...
  while (!defined $sid){
      say "\nSID request failed. Bad connection or proxy not running? Retrying...\n";
      sleep 1;
...
sub create_ticket{
   ...
   my $description;   
   my $ua = new LWP::UserAgent(keep_alive => 1);
   $ua->credentials('servicedesk_hostname:80', '', $username, $password);
   ...
   $ua->default_header('X-Requested-With' => 'SOC Qualys Script');  
#   $ua->show_progress(1);
...
IT_Security_Team_changethis (itsecurity\@mycompany.com_changethis) has done a vulnerability scan on host $$content[0]. In order to remain compliant with PCI - Data Security Standards, it is important that the issue described below is addressed immediately.
...
sub get_group_id{
   ...
   if ($grouphashres->is_success){
      ...
      if (defined $grouphash){
         ...
         say "CUSTOM ASSIGNEE GROUP $$group WAS NOT FOUND. DEFAULTING TO IT_SECURITY_QUEUE_changethis";
         return undef;
      }
   }else{
...
### SOMETIMES, ASSET GROUPS ARE RENAMED. WHEN THIS IS DONE, OLD SCAN RESULTS WITH EMBEDDED ASSETGROUP ARE NO LONGER SEEN BY THE SCRIPT. SET AS MANY MAPPINGS HERE AS YOU WANT.
          $embedded_title = 'NEW_ASSETGROUPTITLE1_changethis' if $embedded_title eq 'OLD_ASSETGROUPTITLE1_changethis';
          $embedded_title = 'NEW_ASSETGROUPTITLE2_changethis' if $embedded_title eq 'OLD_ASSETGROUPTITLE2_changethis';
...
### IF $XMLFOLDER IS BLANK, SET IT TO DEFAULT
       if (ref($xmlfolder)){
          $xmlfolder = $cfg->param("xml_folder", '//path\to\xml scan results'); #changethis
          $xmlfolder = $cfg->param("xml_folder");
       }
...
# UPLOAD EMPTY FOLDER? 
# upload_server IS WHERE SERVICE DESK UPLOADS ATTACHMENTS. FOUND THIS AND ALL THE SEQUENCE BELOW BY DEBUGGING HTTP COMMUNICATION OF MY BROWSER WITH SERVICE DESK APP 
# HOPEFULLY BY MAKING THIS SCRIPT PUBLIC, SOMEONE WILL HELP MODIFY THIS TO COMMUNICATE TO SERVICE DESK VIA WEB SERVICES INSTEAD OF PLAIN HTTP            
$$ua->post(
  'http://upload_server_changethis:8080/CAisd/UploadServlet', 
...
# KT OPEN SESSION
my $opensessionurl = 'http://servicedesk_url_changethis/CAisd/pdmweb.exe?SID=' . $sid . '+FID=7407+OP=KT_OPEN_REP_SESSION+Keep=1+AttmntId=0+FolderId=0+Host=NDCCASDMAPP402+FileName=' . $attachid . '_' . $attachmentname . '+RepId=doc_rep:1002';
...
$$ua->post(
  'http://upload_server_changethis:8080/CAisd/UploadServlet',
   ['inpDocRepository' => $docrep,
    ...
    'inpRetURL' => 'http://servicedesk_url_changethis/CAisd/pdmweb.exe?SID=' . $sid . '+FID=6169+OP=DISPLAY_FORM+HTMPL=attmnt_upload_done.htmpl+KEEP.AttmntParent=' . $persid,
    'Test' => '',
    'inpServerName' => 'UPLOAD_SERVER_changethis',
    'inpMaxFileSize' => 0,
    'inpBpsid' => $inpbpsid,        
  ],
  'Referer' => 'http://servicedesk_url_changethis/CAisd/pdmweb.exe?SID=' . $sid . '+FID=1515+OP=DISPLAY_FORM+HTMPL=attmnt_upload_popup.htmpl+AttmntId=0+RepId=0+FolderId=0+View=Upload+ShowFields=Yes+ShowImgStatus=Yes+ShowRepList=Yes+RepType=0+KEEP.POPUP_NAME=' . $popupname . '+KEEP.PARENT_DIV=nbtab_1+KEEP.attmnt_parent=' . $persid . '+KEEP.use_role=1',
...
# KEEP ALIVE REQUEST
my $keepalive = $$ua->get('http://servicedesk_url_changethis/CAisd/pdmweb.exe?SID=' . $sid . '+FID=1+OP=KEEP_ALIVE+REP_PROCID=rep_daemon:NDCCASDMAPP402+REP_SESSION=' . $inpbpsid,
'Referer' => 'http://servicedesk_url_changethis/CAisd/pdmweb.exe?SID=' . $sid . '+FID=7407+OP=KT_OPEN_REP_SESSION+Keep=1+AttmntId=0+FolderId=0+Host=NDCCASDMAPP402+FileName=' . $attachid . '_' . $attachmentname . '+RepId=doc_rep:1002'
);
...
# KT CLOSE SESSION
my $ktclosesession = $$ua->get('http://servicedesk_url_changethis//CAisd/pdmweb.exe?SID=' . $sid . '+FID=7848+OP=KT_CLOSE_REP_SESSION+Host=NDCCASDMAPP402+SessionId=' . $inpbpsid,
...
### SIMULATE EXCEL's RUNTIME AUTOFIT FEATURE. LEARED THIS FROM SOMEONE'S CODE.. JUST GOOGLE TO FIND IT.
...
sub get_login{
  ...
  $ua->cookie_jar({});
  $ua->default_header('X-Requested-With' => 'Perl Script'); 
  my $launchreq = HTTP::Request->new(POST => 'https://qualysapi.qualys.com/api/2.0/fo/session/');
  $launchreq->content_type('application/x-www-form-urlencoded');
  $launchreq->content("action=login&username=$qualysusername&password=$qualyspassword");
  my $launchres = $ua->request($launchreq);
